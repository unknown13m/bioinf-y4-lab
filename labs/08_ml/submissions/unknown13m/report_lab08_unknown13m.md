...
Ca si diferenta de concept, la LogReg am un model linear (decizie liniara în spatiul feature-urilor), iar la RF am un model non-linear (captura interactiuni intre feature-uri).
...
-daca etichetele (Condition) se aliniază cu clusterele, atunci structura nesupravegheată recuperează parțial clasele
...
6. **LIMITĂRI**
Am avut si limitări de genul nr mic de probe (si asta mi-a cam dat rezultate instabile), un posibil bias din modul de generare a datelor sau din distributii si imi lipseste variabilitatea biologică (tocmai fiindca am un dataset demo).
